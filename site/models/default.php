...
class DdcpssModelsDefault extends JModelBase
{
  ...
  if($this->data['table']=='usermembership'){
  		if($this->data['alias']==null){
  			$row->alias = JFilterOutput::stringURLSafe($this->data['title']);
  		}
  		$exp_date = explode("/", $this->data['expiry_date']);
  		$row->expiry_date = trim($exp_date[2])."-".trim($exp_date[1])."-".trim($exp_date[0])." 00:00:00";
  	}
